...
const TenantsScreen = ({ user }) => {
  ...
  // Auto-open modal when navigated with openModal param
  useEffect(() => {
    if (searchParams.get('openModal') === 'true') {
      setEditingTenant(null);
      setModalOpen(true);
      setSearchParams({ tab: 'tenants' }, { replace: true });
    }
  ...
  const filtered = useMemo(() => {
    ...
    return resultList.filter((t) =>
      [t.name, t.tradingName, t.contactEmail, t.registrationNumber, t.propertyId ? propertyMap.get(t.propertyId) : null]
        .filter(Boolean)
        .some((v) => String(v).toLowerCase().includes(q))
    );
  }, [resultList, searchInput, propertyMap]);
