...
const DashboardScreen = ({ user, onLogout, onReset, initialTab = 'activity' }) => {
  ...
  const [selectedDoc, setSelectedDoc] = useState(null); // Controls the modal
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 font-sans">
      {/* SIDEBAR */}
      <aside className="w-64 bg-slate-900 text-slate-300 flex flex-col hidden lg:flex shrink-0 border-r border-slate-800">
          ...
          <>
            <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 z-10">
              <div className="relative max-w-sm w-full">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border-slate-200 border rounded-xl py-2 pl-12 pr-4 text-sm font-bold outline-none focus:border-indigo-500"
                  value={filterQuery}
              ...
              <div className="flex items-center gap-4">
                ...
                <button
                  ...
                  className="bg-slate-900 text-white px-5 py-2.5 rounded-xl text-sm font-black flex items-center gap-2"
                >
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              ...
              <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-8 py-5 border-b border-slate-100 flex items-center gap-8 bg-slate-50/30">
                  <TabButton
                    active={activeTab === 'activity'}
                    label="Overview"
                    onClick={() => setActiveTab('activity')}
                      ...
                      {tableData.map((doc) => (
                        ...
                        >
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-4">
                              <FileText
                                className={`w-5 h-5 ${doc.status === 'SUCCESS' ? 'text-emerald-500' : 'text-amber-500'}`}
                              />
                          ...
                          <td className="px-8 py-6 font-black text-slate-900">R {doc.amount}</td>
          ...
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <div className="relative w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-in slide-in-from-right duration-300">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                <div className="flex items-center gap-4">
                  <div
                    className={`w-14 h-14 rounded-2xl flex items-center justify-center ${selectedDoc.status === 'SUCCESS' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}
                  >
                    <FileText size={28} />
                  </div>
                  <div>
                    <h3 className="font-black text-slate-900 leading-tight mb-1">{selectedDoc.fileName}</h3>
              ...
              <div className="p-6 border-t bg-slate-50/50 flex gap-3">
                <button className="flex-1 bg-slate-900 text-white py-3 rounded-xl font-black text-sm">
                  Download PDF
                </button>
                {selectedDoc.status === 'FAILED' && (
                  <button className="flex-1 bg-indigo-600 text-white py-3 rounded-xl font-black text-sm">
                    Validate Entry
                  </button>
                )}
              </div>
...
const KPICard = ({ label, value, icon, status = 'default' }) => (
  <div className="bg-white p-7 rounded-[2rem] border border-slate-200 shadow-sm">
    <div className="flex justify-between items-center mb-6">
      <div
        className={`w-12 h-12 rounded-xl flex items-center justify-center ${status === 'success' ? 'bg-emerald-50' : status === 'warning' ? 'bg-amber-50' : 'bg-blue-50'}`}
      >
        {icon}
      </div>
...
const TabButton = ({ active, label, count, onClick }) => (
  <button
    onClick={onClick}
    className={`text-sm font-black pb-4 transition-all relative ${active ? 'text-indigo-600' : 'text-slate-400'}`}
  >
    {label}{' '}
